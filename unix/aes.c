...
static const uint8_t sbox[256] = { //used for key expansion
...
{
  uint8_t *Iv = ctx->Iv;
  //Perform the block cipher on each 16-byte block
  for (uint32_t i = 0; i < length; i += AES_BLOCKLEN) {
    XorWithIv(buf, Iv); //output is stored in buf
    Cipher((state_t*)buf, ctx->RoundKey);
    Iv = buf; //set next XOR value
    buf += AES_BLOCKLEN; //advance the buf pointer
  }
  memcpy(ctx->Iv, Iv, AES_BLOCKLEN);
}
